...
function parseJson(
  ...
  context: { experimentId: number; field: string },
): unknown {
  ...
  try {
    ...
    logger.warn(
      { err, database: "primary", ...context, contentLength: value.length, fallback: "omit_malformed_ai_context", retryExpected: false },
      "Stored experiment JSON could not be parsed and will be omitted from AI context, so generation can continue but may be less grounded. Repair or replace the affected stored field before relying on AI analysis.",
    );
    return null;
  }
}

function compactPlateData(rawDataJson: string | null, experimentId: number): unknown {
  const parsed = parseJson(rawDataJson, { experimentId, field: "raw_data_json" });
...
export function buildExperimentContext(
  ...
  if (options.includeProtocol !== false) {
    context.protocol = experiment.protocol_json
      ? parseStructuredProtocol(experiment.protocol_json) ?? parseJson(
          experiment.protocol_json,
          { experimentId: experiment.id, field: "protocol_json" },
        )
      : null;
  }
  if (options.includeData !== false) context.quantitative_data = compactPlateData(experiment.raw_data_json, experiment.id);
  if (options.includeControls !== false) {
    context.control_summary = parseJson(
      experiment.control_summary_json,
      { experimentId: experiment.id, field: "control_summary_json" },
    );
  }
...
export function buildRelatedExperimentContext(
  ...
  const rows = experimentRows.map((experiment, index) => ({
    ...
    protocol: !options.metadataOnly && experiment.protocol_json
      ...
      : undefined,
    quantitative_data: options.includeData ? compactPlateData(experiment.raw_data_json, experiment.id) : undefined,
