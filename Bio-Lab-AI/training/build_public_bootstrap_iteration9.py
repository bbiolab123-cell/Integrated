#!/usr/bin/env python3
"""Build audited BioLab public-bootstrap iteration 9.

Iteration 9 promotes every iteration-8 holdout to training-only data, adds
...
EXPECTED_ITERATION8_DATASET_SHA256 = (
    "7a19335f825bbe93f7ebdccba9996044a73743b0a4ea636245501a171bf56072"
)
EXPECTED_ITERATION8_REVIEW_SHA256 = (
    "7fd62c79d9f185e4744d4f1c818613f78fa8b9a9981ff148637321b24eefa5cc"
)
EXPECTED_ITERATION8_GATE_REPORT_SHA256 = (
    "fac83d66aa6bcd90c41b5f83bf412d01c85fec1a0ac14357ace97ea8f7053c67"
)
FAILED_ITERATION8_EVALUATION_IDS = (
    "eval-0002-09fb999b0c06",
    "eval-0004-195c63817a3a",
    "eval-0007-3b313be1a2a0",
    "eval-0010-62cc09efa981",
    "eval-0012-821840b36e80",
    "eval-0014-a928ba08fb0e",
    "eval-0015-ac704ddd8c2f",
    "eval-0017-c22627a98748",
    "eval-0018-c3f80b1f0c26",
)
AUDITED_ITERATION8_REVIEW_SUMMARY = {
    "review_sha256": EXPECTED_ITERATION8_REVIEW_SHA256,
    "gate_report_sha256": EXPECTED_ITERATION8_GATE_REPORT_SHA256,
    "records": 20,
...
ITERATION_SCHEMA_VERSION = 8
TOTAL_EXAMPLES = 1080
EXPECTED_SPLITS = Counter({"train": 1040, "validation": 20, "test": 20})
EXPECTED_STRUCTURED_SPLITS = Counter({"train": 264, "validation": 5, "test": 5})
PUBMED_HOLDOUT_DECISIONS = tuple("yes" if index % 2 == 0 else "no" for index in range(18))
PROTOCOL_HOLDOUT_PATHS = {
    "validation": {
        "protocol_generation": (
            "markdown-output/general-genotyping-of-myzus-persicae-using-pcr-wit-c4tnywme.md"
        ),
        "sop_structuring": (
            "markdown-output/fixation-and-staining-of-gemmule-hatched-ephydatia-cnzdvf26.md"
        ),
    },
    "test": {
        "protocol_generation": (
            "markdown-output/small-volume-fungal-genomic-dna-extraction-protoco-3w9gph6.md"
        ),
        "sop_structuring": (
            "markdown-output/protocol-to-process-gastruloids-for-facs-reporters-bvgrn3v6.md"
        ),
    },
}
EXACTNESS_RULE = (
    "Iteration 9 release rule: Return only the requested format. For decision text, "
...
STRUCTURED_RULE = (
    "Iteration 9 JSON rule: Emit one complete JSON object and nothing else. Use every "
...
def iteration9_prompt(user: str, *, focus_index: int, structured: bool) -> str:
...
def resolve_failed_hashes(parent_rows: list[dict[str, Any]]) -> list[str]:
    ...
    failures = []
    for evaluation_id in FAILED_ITERATION8_EVALUATION_IDS:
...
def make_variant(builder, iteration3_builder, original: dict[str, Any], *, focus_index: int) -> dict[str, Any]:
    ...
    variant = iteration3_builder.make_row(
        ...
        task=task,
        split="train",
        user_content=iteration9_prompt(base_user, focus_index=focus_index, structured=structured),
...
def build_focused_variants(builder, iteration3_builder, parent_rows, failed_hashes):
    ...
    for bucket in buckets.values():
        bucket.sort(key=lambda row: sha256_text(f"iteration9-focus:{row['example_hash']}"))
    ...
    def add(original, focus_index: int, role: str) -> None:
        ...
        if variant["input_hash"] in input_hashes:
            raise AssertionError(f"Duplicate iteration-9 focused input for {role}.")
    ...
    for task in (task for task in builder.PUBMED_TASKS if task != "experiment_analysis"):
        for decision in ("yes", "no", "maybe"):
            ...
            for replay_index in range(3):
                add(sources[replay_index], focus_index, "iteration9_balanced_decision_replay")
                focus_index += 1

    for failed_hash in failed_hashes:
        for _ in range(4):
            add(by_hash[failed_hash], focus_index, "iteration9_failed_case_replay")
            focus_index += 1
    ...
    for decision in ("yes", "no", "maybe"):
        ...
        for replay_index in range(5):
            add(eligible[replay_index], focus_index, "iteration9_structured_analysis_replay")
            focus_index += 1

    for task in builder.PROTOCOL_TASKS:
        sources = sorted(
            (row for row in parent_rows if row["task_type"] == task),
            key=lambda row: sha256_text(f"iteration9-protocol:{row['example_hash']}"),
        )
        replay_count = 8 if task == "protocol_generation" else 7
        for replay_index in range(replay_count):
            add(sources[replay_index], focus_index, "iteration9_protocol_schema_replay")
            focus_index += 1

    expected = Counter({
        "iteration9_balanced_decision_replay": 54,
        "iteration9_failed_case_replay": 36,
        "iteration9_structured_analysis_replay": 15,
        "iteration9_protocol_schema_replay": 15,
    })
    if len(variants) != 120 or Counter(item["training_role"] for item in metadata.values()) != expected:
        raise AssertionError("Iteration-9 focused replay plan is incomplete.")
...
def build_pubmed_holdouts(builder, iteration2_builder, iteration3_builder, used_pubids):
    ...
    for decision, count in required.items():
        buckets[decision].sort(key=lambda row: sha256_text(f"biolab-pubmed-iteration9-selection-v1:{row['pubid']}"))
            ...
            for _ in range(task_counts()[task]):
                ...
                while True:
                    ...
                    candidate_prompt = "\n".join([
                        ...
                        iteration9_prompt(strict_user, focus_index=0, structured=True),
                    ])
                    if not iteration3_builder.unsupported_numbers(candidate_target, candidate_prompt):
                        break
                user = iteration9_prompt(
                    strict_user,
                ...
                source_manifest.append({
                    ...
                    "response_contract_version": "iteration9_schema_fidelity_v1",
                })
    if len(rows) != 36:
        raise AssertionError("Iteration-9 PubMedQA holdout count failed.")
...
def build_protocol_holdouts(builder, iteration2_builder, iteration3_builder, used_identifiers):
    ...
    for split, task_paths in PROTOCOL_HOLDOUT_PATHS.items():
        for task, path in task_paths.items():
            if path in used_identifiers:
                raise AssertionError(f"Iteration-9 protocol holdout was already used: {path}")
            ...
            row = iteration3_builder.make_row(
                ...
                user_content=iteration9_prompt(strict_user, focus_index=len(rows), structured=True),
                assistant_content=target,
            )
            updated = dict(item)
            updated.update({
    ...
    if len(rows) != 4:
        raise AssertionError("Iteration-9 protocol holdout count failed.")
    return rows, source_manifest
...
def validate_review_inputs(review_path: Path | None, gate_report_path: Path | None) -> None:
    if review_path is not None:
        ...
        if actual != EXPECTED_ITERATION8_REVIEW_SHA256:
            raise AssertionError(f"Iteration-8 review changed: expected {EXPECTED_ITERATION8_REVIEW_SHA256}, found {actual}.")
        review = json.loads(review_path.read_text(encoding="utf-8"))
        if not isinstance(review, list) or len(review) != 20:
            raise AssertionError("Iteration-8 review must contain exactly 20 ratings.")
    if gate_report_path is not None:
        actual = sha256_bytes(gate_report_path.read_bytes())
        if actual != EXPECTED_ITERATION8_GATE_REPORT_SHA256:
            raise AssertionError(f"Iteration-8 gate report changed: expected {EXPECTED_ITERATION8_GATE_REPORT_SHA256}, found {actual}.")
        ...
        if report.get("release_accepted") or report.get("disposition") != "quarantined":
            raise AssertionError("Iteration-8 gate report is not the quarantined result.")
        if report.get("passed_gate_count") != 3 or report.get("gate_count") != 5:
            raise AssertionError("Iteration-8 gate totals do not match the audited result.")
        if tuple(report.get("failed_adapter_evaluation_ids", ())) != FAILED_ITERATION8_EVALUATION_IDS:
            raise AssertionError("Iteration-8 failed evaluation IDs changed.")
...
def build_iteration(builder, iteration2_builder, iteration3_builder, iteration4_builder, parent_rows, parent_manifest):
    ...
    if len(manifest_examples) != len(rows):
        raise AssertionError("Iteration-9 manifest/example count mismatch.")
    return rows, {
        "iteration_schema_version": ITERATION_SCHEMA_VERSION,
        "dataset_schema_version": builder.DATASET_SCHEMA_VERSION,
        "parent_dataset_sha256": EXPECTED_ITERATION8_DATASET_SHA256,
        "parent_review_sha256": EXPECTED_ITERATION8_REVIEW_SHA256,
        "parent_gate_report_sha256": EXPECTED_ITERATION8_GATE_REPORT_SHA256,
        "reason": (
            "Iteration 8 passed structured validity, privacy, and improvement gates but "
            "failed measurement fidelity and the 80% held-out quality gate. Every prior "
            "holdout is now training-only; 120 source-grounded corrective replays and fresh "
            "unseen holdouts target those failures."
        ),
        "holdout_note": (
            "Unused PubMedQA maybe groups remain exhausted, so fresh iteration-9 PubMedQA "
        ...
        "failed_adapter_evaluation_ids": list(FAILED_ITERATION8_EVALUATION_IDS),
        "review_summary": dict(AUDITED_ITERATION8_REVIEW_SUMMARY),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--iteration8-jsonl", type=Path, required=True)
    parser.add_argument("--iteration8-manifest", type=Path, required=True)
    parser.add_argument("--iteration8-review", type=Path)
    parser.add_argument("--iteration8-gate-report", type=Path)
...
def main() -> int:
    ...
    parent_rows, parent_bytes = load_jsonl(args.iteration8_jsonl)
    actual_sha = sha256_bytes(parent_bytes)
    if actual_sha != EXPECTED_ITERATION8_DATASET_SHA256:
        raise AssertionError(f"Iteration-8 dataset changed: expected {EXPECTED_ITERATION8_DATASET_SHA256}, found {actual_sha}.")
    parent_manifest = json.loads(args.iteration8_manifest.read_text(encoding="utf-8"))
    if parent_manifest["dataset_sha256"] != actual_sha:
        raise AssertionError("Iteration-8 manifest does not match its JSONL.")
    validate_review_inputs(args.iteration8_review, args.iteration8_gate_report)
